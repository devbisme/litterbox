...
#
# Modified by Dave Vandenbout as follows:
#
#   * The write and transfer methods will check the data to be sent and
#     will not convert it if it is already in the form of a string of bytes.
#
#   * The write, read, and transfer methods will partition the data into
#     4096 byte packets which seems to be the limit for the Raspberry Pi 3
#     SPI driver/hardware. (See BUFF_SIZE below.)
#
#   * The write, read, and transfer methods advance a pointer along the 
#     data buffer rather than making copies of each packet.
#
...
class SPI(object):
    ...
    BUFF_SIZE = 4096  # Maximum length of single SPI transfer for RPi 3.
    ...
    def write(self, data, speed=0, bits_per_word=0, delay=0):
        ...
        for i in range(0, length, BUFF_SIZE):
            buff_size = BUFF_SIZE if i < length-BUFF_SIZE else length-i
            spi_ioc_transfer = struct.pack(SPI._IOC_TRANSFER_FORMAT,
                                       ctypes.addressof(transmit_buffer)+i, 0,
                                       buff_size, speed, delay, bits_per_word, 0,
                                       0, 0, 0)
    ...
    def read(self, length, speed=0, bits_per_word=0, delay=0):
        ...
        for i in range(0, length, BUFF_SIZE):
            ...
            spi_ioc_transfer = struct.pack(SPI._IOC_TRANSFER_FORMAT, 0,
                                       ctypes.addressof(receive_buffer)+i,
                                       buff_size, speed, delay, bits_per_word, 0,
                                       0, 0, 0)
            fcntl.ioctl(self.handle, SPI._IOC_MESSAGE, spi_ioc_transfer)
    ...
    def transfer(self, data, speed=0, bits_per_word=0, delay=0):
        ...
        for i in range(0, length, BUFF_SIZE):
            buff_size = BUFF_SIZE if i < length-BUFF_SIZE else length-i
            spi_ioc_transfer = struct.pack(SPI._IOC_TRANSFER_FORMAT,
                                       ctypes.addressof(transmit_buffer)+i,
                                       ctypes.addressof(receive_buffer)+i,
                                       buff_size, speed, delay, bits_per_word, 0,
                                       0, 0, 0)
            fcntl.ioctl(self.handle, SPI._IOC_MESSAGE, spi_ioc_transfer)
